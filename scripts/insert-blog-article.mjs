/**
 * Jednorazowy skrypt do wgrania artykułu do Supabase.
 * Uruchom: node scripts/insert-blog-article.mjs
...
if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
  ...
  console.error("Dodaj do .env.local: SUPABASE_SERVICE_ROLE_KEY=twoj_klucz");
  process.exit(1);
}
...
const article = {
  ...
  title: "Strona internetowa dla dentysty - co <em>musi zawierać</em> w 2026",
  excerpt:
...
<p>Jeśli prowadzisz gabinet i chcesz wiedzieć, co konkretnie blokuje Twoją widoczność lub dlaczego pacjenci nie zostawiają zapytań - <a href="/kontakt" class="link">napisz, zróbmy bezpłatny audyt</a>. W ciągu 48 godzin dostajesz konkretną listę rzeczy do poprawy, nie ogólnikowy raport z podlinkowanym cennikiem.</p>`,
  author: "Stanisław Blicharski",
  published_at: "2026-07-28T08:00:00.000Z",
  category: "Strony dla firm",
  tags: ["Strony dla firm", "Branże", "SEO", "Stomatologia", "Konwersja", "Next.js"],
  image_url: null,
...
if (error) {
  console.error("Błąd wgrywania:", error.message);
  process.exit(1);
}

console.log("✓ Artykuł wgrany:");
console.log("  slug:", data.slug);
console.log("  URL: https://mainly.pl/blog/" + data.slug);
